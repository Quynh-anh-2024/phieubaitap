...
import { GradeLevel as GradeLevelType } from '../types'; // Fix import conflict if needed or use direct
...
const WorksheetForm: React.FC<WorksheetFormProps> = ({ onSubmit, isLoading }) => {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl p-5 md:p-10 border border-slate-100 max-w-2xl mx-auto transform transition-all hover:shadow-2xl relative overflow-hidden">
      {/* Decorative top border */}
      <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-teal-400 via-emerald-400 to-teal-400"></div>

      <div className="flex items-center gap-3 mb-6 md:mb-8 text-teal-700">
        <div className="p-2 bg-teal-50 rounded-lg">
             <Layers className="w-6 h-6" />
        </div>
        <h2 className="text-xl md:text-2xl font-bold">Thông Tin Phiếu Bài Tập</h2>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 md:space-y-8">
          ...
          <div className="grid grid-cols-3 gap-3 md:gap-4">
            {Object.values(GradeLevel).map((g) => (
              <button
                ...
                type="button"
                onClick={() => setGrade(g)}
                className={`py-3 px-2 md:px-4 rounded-xl border-2 font-bold text-sm md:text-lg transition-all duration-200 ${
                  grade === g
        ...
        <div>
          ...
          <div className="relative">
            <select
                ...
                className="w-full p-3 md:p-4 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none transition-all text-slate-700 font-semibold text-base md:text-lg appearance-none cursor-pointer hover:bg-white"
        ...
        {subject === 'Tiếng Việt' && (
            <div className="animate-fade-in bg-slate-50 p-3 md:p-4 rounded-xl border border-slate-100">
                <label className="block text-sm font-bold text-slate-700 mb-3 flex items-center gap-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <button
                        ...
                        className={`flex items-center justify-center gap-2 py-3 px-3 rounded-lg border font-semibold text-sm md:text-base transition-all ${
        ...
        <div>
          ...
          <input
            ...
            className="w-full p-3 md:p-4 bg-white border-2 border-slate-200 rounded-xl focus:ring-0 focus:border-teal-500 outline-none transition-all placeholder:text-slate-400 text-base md:text-lg shadow-sm"
            required
          />
        </div>
        ...
        <button
          ...
          className={`w-full py-3 md:py-4 rounded-xl font-bold text-white text-base md:text-lg shadow-lg transform transition-all hover:-translate-y-0.5 active:translate-y-0 active:scale-[0.99] ${
        ...
        >
          {isLoading ? (
            ...
            <span className="flex items-center justify-center gap-2">
                ...
                {exerciseType === 'reading' ? 'Tạo Phiếu Đọc Hiểu & Kiến Thức' : 'Tạo Phiếu Bài Tập'}
            </span>
          )}
        </button>
      </form>
